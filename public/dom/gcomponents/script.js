...
class ClassSlider {
  constructor(options) {
    ...
    this.pixelsPerValue = (this.elem.clientWidth - this.toggle.clientWidth) / this.max;
  ...
  endDrag() {
    ...
    this.elem.dispatchEvent(new CustomEvent('change', {
      bubbles: true,
      detail: this.positionToValue(this.newLeft)
    }));
  }

  moveAt(pageX) {
    this.newLeft = pageX - this.shiftX - this.elem.getBoundingClientRect().left;
    let maxRight = this.elem.offsetWidth - this.toggle.offsetWidth;

    if (this.newLeft < 0) {
      this.newLeft = 0;
    }
    if (this.newLeft > maxRight) {
      this.newLeft = maxRight;
    }

    this.toggle.style.left = this.newLeft + 'px';
  ...
  setValue(value) {
    this.newLeft = this.valueToPosition(value);
    this.toggle.style.left = this.newLeft + 'px';
